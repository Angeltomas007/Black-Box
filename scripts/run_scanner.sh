#!/bin/bash
# Linux launcher: chmod +x run_scanner.sh puis double-clique (ou lance
# ./run_scanner.sh dans un terminal). Rien n'est mis en ligne : le site
# tourne uniquement sur cet ordinateur (http://localhost:8501).
set -e
cd "$(dirname "$0")"
...
if ! command -v git &> /dev/null; then
    echo "[ERREUR] Git n'est pas installe. Installe-le via ton gestionnaire de paquets"
    echo "(ex: sudo apt install git)."
...
if [ -z "$PYTHON_BIN" ]; then
    echo "[ERREUR] Python 3 n'est pas installe (ex: sudo apt install python3 python3-venv)."
    read -p "Appuie sur Entree pour fermer..."
    exit 1
fi
...
echo "Ouvre http://localhost:8501 dans ton navigateur si ca ne s'ouvre pas seul."
